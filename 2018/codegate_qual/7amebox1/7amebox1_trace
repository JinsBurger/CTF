...
class EMU:
    ...
    def execute(self):
        try:
            while 1:
                ...
                print(op_handler.__name__ + "({0},{1}".format(op_type,opers)+")")
    ...
    def op_x8(self, op_type, opers):
        ...
        if 0 <= syscall_num < len(self.syscall_table):
            ...
            print("call syscall function : " + syscall.__name__)
            syscall()
    ...
    def sys_s2(self):
        ...
        print("(fd,buf,size) = ({0},{1},{2})".format(fd,buf,size))
    ...
    def sys_s3(self): #read
        ...
        if 0 <= fd < len(self.pipeline):
            ...
            print(self.read_memory(buf, size))
